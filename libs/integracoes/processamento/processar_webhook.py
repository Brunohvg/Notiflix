...
def processar_eventos(store_id, event_type, order_id):
    ...
    try:
        ...
        if not novo_pedido:
            logger.error("Não foi possível recuperar o pedido da API. Pedido não processado: %s", order_id)
            return False
    ...
    try:
        ...
        if not Pedido.objects.filter(id_venda=id_venda).exists():
            ...
            cliente, created = Cliente.objects.get_or_create(
                contact_email=cliente_dados["contact_email"],
                defaults=cliente_dados
            )
